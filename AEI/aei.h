...
#include "aei_global.h"

#pragma once

#include "alphaInstruction.h"#include "../Core/InstructionHelpers.h"
#include <QtGlobal>
#include "../AESH/AlphaMemorySystem.h"
...
//==============================================================================
// BIT COUNTING INSTRUCTIONS (BWX Extension - EV67+)
//==============================================================================

/**
 * @brief Count Leading Zeros (CTLZ)
 * Counts the number of zero bits from the most significant bit
 */
class CtlzInstruction : public IntegerInstruction {
public:
    explicit CtlzInstruction(quint32 rawInstr);

    void Execute(RegisterFileWrapper* regs, AlphaMemorySystem* memSys, TLBSystem* tlb) override;
    const char* GetMnemonic() const override { return "CTLZ"; }
...
/**
 * @brief Count Population (CTPOP)
 * Counts the number of set bits (1s) in the operand
 */
class CtpopInstruction : public IntegerInstruction {
public:
    explicit CtpopInstruction(quint32 rawInstr);

    void Execute(RegisterFileWrapper* regs, AlphaMemorySystem* memSys, TLBSystem* tlb) override;
    const char* GetMnemonic() const override { return "CTPOP"; }
...
/**
 * @brief Count Trailing Zeros (CTTZ)
 * Counts the number of zero bits from the least significant bit
 */
class CttzInstruction : public IntegerInstruction {
public:
    explicit CttzInstruction(quint32 rawInstr);

    void Execute(RegisterFileWrapper* regs, AlphaMemorySystem* memSys, TLBSystem* tlb) override;
    const char* GetMnemonic() const override { return "CTTZ"; }
