...
private:
	...
	void execConditionalBranch(const BranchInstruction& br, std::function<bool(qint64)> condition) {
		qint64 raVal = static_cast<qint64>(regs->readIntReg(br.ra));
		quint64 target = alphaCPU->getPC() + static_cast<qint64>(br.displacement << 2);
	...
	void execBR(const BranchInstruction& br) {
		quint64 newPC = alphaCPU->getPC() + static_cast<qint64>(br.displacement << 2);
		regs->writeIntReg(br.ra, alphaCPU->getPC());
