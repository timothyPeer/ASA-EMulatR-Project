...
class AlphaExecutor : public QObject {
	Q_OBJECT
public:
	AlphaExecutor(AlphaCPUInterface* alphaCPU_, AlphaMemorySystem* memSys, RegisterBank* regs, FpRegisterBankcls* fpRegs_, QObject* parent = nullptr)
		: QObject(parent), alphaCPU(alphaCPU_), memory(memSys), regs(regs), fpRegs_alphaExecutor(fpRegs_){
	}
...
public:
	explicit IntegerExecutor(AlphaCPUInterface* cpuptr, 
		AlphaMemorySystem* mem,
		RegisterBank* regs, 
...
signals:
	...
	void illegalInstruction(quint64 instructionWord, quint64 pc);
public:
	explicit FloatingPointExecutor(AlphaCPUInterface* cpuptr, AlphaMemorySystem* mem, RegisterBank* regs, FpRegisterBankcls* fpRegs__, QObject* parent = nullptr)
...
public:
	explicit VectorExecutor(AlphaCPUInterface* cpuptr, AlphaMemorySystem* mem, RegisterBank* regs,FpRegisterBankcls* fpRegs, QObject* parent = nullptr)
		: AlphaExecutor(cpuptr, mem, regs, fpRegs, parent) {
		buildDispatchTable();
	}
...
public:
	explicit ControlExecutor(AlphaCPUInterface* cpuPtr, AlphaMemorySystem* mem, RegisterBank* regs, FpRegisterBankcls* fpRegs, QObject *parent = nullptr)
		: AlphaExecutor(cpuPtr, mem, regs,fpRegs, parent)
	{
		buildDispatchTable();
	}
