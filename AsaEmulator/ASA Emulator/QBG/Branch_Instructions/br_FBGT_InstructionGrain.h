#ifndef BR_FBGT_INSTRUCTIONGRAIN_H
#define BR_FBGT_INSTRUCTIONGRAIN_H
...
/** *********************************************************************************************
 * @brief br_FBGT (Floating-Point Branch if Greater Than Zero)
 *
 * Architectural Opcode: 0x37
 * Architectural Function Code: 0x00 (bits <5:0> of instruction word)
 *
 * Description:
 * Floating-point conditional branch instruction.
 * Branches to the target address if the FPCC_GT (Floating Point Condition Code - Greater Than Zero)
 * bit in the Floating-Point Condition Register (FPCR) is set (FPCC_GT = 1).
 * The target address is calculated as:  
 * target_PC = current_PC + (signed 21-bit displacement << 2)
 *
 * Implementation Notes:
 * - Opcode: 0x37 (per ASA Table C-5)
 * - Function code: 0x00
 * - Displacement field: bits <20:0>, sign-extended, left-shifted by 2 for byte offset.
 * - Condition check: Test FPCR GT condition flag.
 *
 ...
 ********************************************************************************************* */
class br_FBGT_InstructionGrain : public AlphaInstructionGrain
{
public:
    /**
     * @brief Executes the br_FBGT instruction.
     *
     * @param cpu        AlphaCPU instance (for PC update and FP condition check)
     * @param context    #include "AlphaProcessorContext.h" (registers and FPCR state)
	...
	void execute(AlphaCPU* cpu, AlphaProcessorContext* context, quint64 rawInstr) const override
	{
		...
		// Confirm FPCC_GT condition reflects correct result of prior FP comparison
		bool conditionMet = context->fpcr().getFPConditionGreaterThan();

		// Displacement properly sign-extended and shifted
		...
		if (conditionMet) {
			...
			cpu->log(QString("br_FBGT taken: PC=0x%1 ? Target=0x%2 (FPCC_GT=1)")
				.arg(pc, 0, 16)
				.arg(targetAddress, 0, 16), LogLevel::CONFIGURATION);
		}
		else {
			context->setPC(pc + 4);
			cpu->log(QString("br_FBGT not taken: PC=0x%1 (FPCC_GT=0)")
		...
		// Advance PC after execution to maintain convention
		context->setPC(context->getPC() + 4);
	}


    /**
     * @brief Decodes the br_FBGT instruction.
     *
     * @param rawInstr   Raw 64-bit instruction word
     ...
     *
     * @return QPair where .first = opcode (0x37), .second = function code (0x00)
     */
    ...
    /**
     * @brief Returns the opcode for br_FBGT.
     * @return Opcode (0x37)
     */
    static constexpr quint8 opcode() { return 0x37; }

    /**
     * @brief Returns the function code for br_FBGT.
     ...
     */
    GrainType grainType() const override { return GrainType::BR_FBGT; }

    /**
     * @brief Returns the mnemonic name for disassembly and debug output.
     */
    QString mnemonic() const override { return "br_FBGT"; }
...
private:
    ...
    /**
     * TODO:
     * - Confirm FPCC_GT condition flag reflects correct result from prior FP compare.
     * - Add test case for positive, zero, and NaN comparisons for FPGT branching.
     */
};

#endif // BR_FBGT_INSTRUCTIONGRAIN_H
