#ifndef BR_BGE_INSTRUCTIONGRAIN_H
#define BR_BGE_INSTRUCTIONGRAIN_H
...
/** *********************************************************************************************
 * @brief br_BGE (Branch if Greater Than or Equal to Zero)
 *
 * Architectural Opcode: 0x3E
 ...
 ********************************************************************************************* */
class br_BGE_InstructionGrain : public AlphaInstructionGrain
{
public:
    /**
     * @brief Executes the br_BGE instruction.
     *
	...
	void execute(AlphaCPU* cpu, AlphaProcessorContext* context, quint64 rawInstr) const override
	{
		...
		if (branchTaken) {
			...
			context->setPC(pc + 4);
			cpu->log(QString("br_BGE not taken: PC=0x%1 (R%2=%3 < 0)")
				.arg(pc, 0, 16)
				.arg(m_ra)
		...
		// Final PC advance to ensure convention compliance
		context->setPC(context->getPC() + 4);
	}


    /**
     * @brief Decodes the br_BGE instruction.
     *
     * @param rawInstr   Raw 64-bit instruction word
     ...
     *
     * @return QPair where .first = opcode (0x3E), .second = function code (0x00)
     */
    ...
    /**
     * @brief Returns the opcode for br_BGE.
     * @return Opcode (0x3E)
     */
    static constexpr quint8 opcode() { return 0x3E; }

    /**
     * @brief Returns the function code for br_BGE.
     ...
     */
    GrainType grainType() const override { return GrainType::BR_BGE; }

    /**
     * @brief Returns the mnemonic name for disassembly and debug output.
     */
    QString mnemonic() const override { return "br_BGE"; }
...
#endif // BR_BGE_INSTRUCTIONGRAIN_H
