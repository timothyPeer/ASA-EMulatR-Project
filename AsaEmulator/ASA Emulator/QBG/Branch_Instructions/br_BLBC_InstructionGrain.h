#ifndef BR_BLBC_INSTRUCTIONGRAIN_H
#define BR_BLBC_INSTRUCTIONGRAIN_H
...
/** *********************************************************************************************
 * @brief br_BLBC (Branch on Low Bit Clear)
 *
 * Architectural Opcode: 0x38
 * Architectural Function Code: 0x00 (bits <5:0> of instruction word)
 *
 * Description:
 * Conditional branch instruction.
 * Branches to the target address if the least significant bit (bit 0) of integer register Ra is clear (0).
 * The target address is calculated as:
 * target_PC = current_PC + (signed 21-bit displacement << 2)
 *
 * Implementation Notes:
 * - Opcode: 0x38 (per ASA Table C-5)
 * - Function code: 0x00
...
class br_BLBC_InstructionGrain : public AlphaInstructionGrain
{
public:
    /**
     * @brief Executes the br_BLBC instruction.
     *
    ...
    void execute(AlphaCPU* cpu, AlphaProcessorContext* context, quint64 rawInstr) const override
    {
        ...
        if (branchTaken)
        {
            ...
            cpu->log(QString("br_BLBC taken at PC=0x%1 ? Target=0x%2 (R%3 bit0 clear)")
                .arg(pc, 0, 16)
        ...
        {
            context->setPC(pc + 4);
            cpu->log(QString("br_BLBC not taken at PC=0x%1 (R%2=0x%3)")
                .arg(pc, 0, 16)
                .arg(m_ra)
    ...
    /**
     * @brief Decodes the br_BLBC instruction.
     *
     * @param rawInstr   Raw 64-bit instruction word
     ...
     *
     * @return QPair where .first = opcode (0x38), .second = function code (0x00)
     */
    ...
    /**
     * @brief Returns the opcode for br_BLBC.
     * @return Opcode (0x38)
     */
    static constexpr quint8 opcode() { return 0x38; }

    /**
     * @brief Returns the function code for br_BLBC.
     ...
     */
    GrainType grainType() const override { return GrainType::BR_BLBC; }

    /**
     * @brief Returns the mnemonic name for disassembly and debug output.
     */
    QString mnemonic() const override { return "br_BLBC"; }
...
private:
    ...
    /**
     * TODO:
     * - Validate displacement sign-extension for full 21-bit range.
     * - Confirm correct LSB check behavior on zero vs non-zero conditions.
     */
};

#endif // BR_BLBC_INSTRUCTIONGRAIN_H
