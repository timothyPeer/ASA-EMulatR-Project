#ifndef BR_BLBS_INSTRUCTIONGRAIN_H
#define BR_BLBS_INSTRUCTIONGRAIN_H
...
/** *********************************************************************************************
 * @brief br_BLBS (Branch on Low Bit Set)
 *
 * Architectural Opcode: 0x3C
 * Architectural Function Code: 0x00 (bits <5:0> of instruction word)
 *
 * Description:
 * Conditional branch instruction.
 * Branches to the target address if the least significant bit (bit 0) of integer register Ra is set (1).
 * The target address is calculated as:  
 * target_PC = current_PC + (signed 21-bit displacement << 2)
 *
 * Implementation Notes:
 * - Opcode: 0x3F (per ASA Table C-5)
 * - Function code: 0x00
...
class br_BLBS_InstructionGrain : public AlphaInstructionGrain
{
public:
    /**
     * @brief Executes the br_BLBS instruction.
     *
	...
	void execute(AlphaCPU* cpu, AlphaProcessorContext* context, quint64 rawInstr) const override
	{
		...
		// Validate displacement and compute target
		...
		if (branchTaken) {
			...
			cpu->log(QString("br_BLBS taken: PC=0x%1 ? Target=0x%2 (R%3 bit0=1)")
				.arg(pc, 0, 16).arg(targetAddress, 0, 16).arg(m_ra), LogLevel::CONFIGURATION);
		}
		else {
			context->setPC(pc + 4);
			cpu->log(QString("br_BLBS not taken: PC=0x%1 (R%2=0x%3, bit0=0)")
		...
		// Advance PC after execution (even though already updated above, as per convention)
		context->setPC(context->getPC() + 4);
	}


    /**
     * @brief Decodes the br_BLBS instruction.
     *
     * @param rawInstr   Raw 64-bit instruction word
     ...
     *
     * @return QPair where .first = opcode (0x3C), .second = function code (0x00)
     */
    ...
    /**
     * @brief Returns the opcode for br_BLBS.
     * @return Opcode (0x3F)
     */
    static constexpr quint8 opcode() { return 0x3C; }

    /**
     * @brief Returns the function code for br_BLBS.
