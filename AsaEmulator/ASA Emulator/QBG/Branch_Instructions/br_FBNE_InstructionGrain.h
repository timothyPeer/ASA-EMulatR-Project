#ifndef BR_FBNE_INSTRUCTIONGRAIN_H
#define BR_FBNE_INSTRUCTIONGRAIN_H
...
/** *********************************************************************************************
 * @brief br_FBNE (Floating-Point Branch if Not Equal to Zero)
 *
 * Architectural Opcode: 0x35
 * Architectural Function Code: 0x00 (bits <5:0> of instruction word)
 *
 * Description:
 * Floating-point conditional branch instruction.
 * Branches to the target address if the FPNE (Floating Branch Not Equal) condition code
 * in the Floating-Point Condition Register (FPCR) is set (FPCC_NE = 1).
 * The target address is calculated as:  
 * target_PC = current_PC + (signed 21-bit displacement << 2)
 *
 * Implementation Notes:
 * - Opcode: 0x35 (per ASA Table C-5)
 * - Function code: 0x00
 * - Displacement field: bits <20:0>, sign-extended, left-shifted by 2 for byte offset.
 * - Condition check: Test FPCR NE condition flag.
 *
...
class br_FBNE_InstructionGrain : public AlphaInstructionGrain
{
public:
    /**
     * @brief Executes the br_FBNE instruction.
     *
	...
	void execute(AlphaCPU* cpu, AlphaProcessorContext* context, quint64 rawInstr) const override
	{
		...
		// Evaluate FPCR FPCC_NE (≠) → confirm correct logic: !(FPCC_EQ)
		bool conditionMet = context->fpcr().getFPConditionNotEqual();

		// Properly sign-extend and shift the displacement
		...
		if (conditionMet) {
			...
			cpu->log(QString("br_FBNE taken: PC=0x%1 → Target=0x%2 (FPCC_NE=1)")
				.arg(pc, 0, 16)
				.arg(targetAddress, 0, 16), LogLevel::CONFIGURATION);
		}
		else {
			context->setPC(pc + 4);
			cpu->log(QString("br_FBNE not taken: PC=0x%1 (FPCC_NE=0)")
    ...
    /**
     * @brief Decodes the br_FBNE instruction.
     *
     * @param rawInstr   Raw 64-bit instruction word
     ...
     *
     * @return QPair where .first = opcode (0x35), .second = function code (0x00)
     */
    ...
    /**
     * @brief Returns the opcode for br_FBNE.
     * @return Opcode (0x35)
     */
    static constexpr quint8 opcode() { return 0x35; }

    /**
     * @brief Returns the function code for br_FBNE.
     ...
     */
    GrainType grainType() const override { return GrainType::BR_FBNE; }

    /**
     * @brief Returns the mnemonic name for disassembly and debug output.
     */
    QString mnemonic() const override { return "br_FBNE"; }
...
private:
    ...
    /**
     * TODO:
     * - Confirm correct FPCC_NE evaluation logic for all FP compare scenarios.
     * - Add unit tests covering NaN, zero, and normal float comparison edge cases.
     */
};

#endif // BR_FBNE_INSTRUCTIONGRAIN_H
