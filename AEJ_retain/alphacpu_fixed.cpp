	...
	m_jitThreshold(50)
{
    ...
    intRegisterBank.reset(new RegisterBank(this));
    fpRegisterBank.reset(new FpRegisterBankcls(this));
    
  
    // configure the executors
	floatingpointExecutor.reset(new FloatingPointExecutor(this, m_memorySystem, parent));
	integerExec.reset(new IntegerExecutor(this, m_memorySystem, parent));
	controlExecutor.reset(new ControlExecutor(this, m_memorySystem,  parent));
	vecExec.reset(new VectorExecutor(this, m_memorySystem,  parent));

    m_jitProfiler.reset(new AlphaJITProfiler(this));
...
{
	// FloatingPointExecutor → AlphaCPU
// 	QObject::connect(floatingpointExecutor.data(), &FloatingPointExecutor::trapRaised,
// 		this, &AlphaCPU::handleFpTrapRaised);
//     / IntegerExecutor → AlphaCPU
//     // ControlExecutor → AlphaCPU
//     // VectorExecutor → AlphaCPU (if trap/illegal signals exist)
       // this is emitted via the IExecutionContext ctx - notifyTrapRaised
	QObject::connect(this, &AlphaCPU::trapRaised, this, &AlphaCPU::handleFpTrapRaised);
	QObject::connect(this, &AlphaCPU::illegalInstruction, this, &AlphaCPU::handleIllegalInstruction);
	QObject::connect(this, &AlphaCPU::fpcrChanged,
		floatingpointExecutor.data(), &FloatingPointExecutor::setFPCR);

	// ControlExecutor → AlphaCPU
// 	QObject::connect(this, &AlphaCPU::trapRaised, this, &AlphaCPU::handleRaisedTrap);
// 	QObject::connect(this, &AlphaCPU::illegalInstruction,
// 		this, &AlphaCPU::handleIllegalInstruction);

	// IntegerExecutor → AlphaCPU
// 	QObject::connect(this, &AlphaCPU::trapRaised,
// 		this, &AlphaCPU::handleTrapRaised);
// 	QObject::connect(integerExec.data(), &IntegerExecutor::illegalInstruction,
// 		this, &AlphaCPU::handleIllegalInstruction);

	// VectorExecutor → AlphaCPU (if trap/illegal signals exist)
// 	QObject::connect(vecExec.data(), &VectorExecutor::trapRaised,
// 		this, &AlphaCPU::handleTrapRaised); // Only if signal defined

	// Memory → AlphaCPU
	QObject::connect(m_memory, &SafeMemory::memoryWritten,
		this, &AlphaCPU::handleMemoryWrite);
	QObject::connect(m_memory, &SafeMemory::memoryRead,
		this, &AlphaCPU::handleMemoryRead);

	// Self-signals (for external observation)
	QObject::connect(this, &AlphaCPU::resetRequested,
...
void AlphaCPU::initialize()
{
	...
	void AlphaCPU::resetCPU()
	{
		...
		if (m_jitCompiler) m_jitCompiler = nullptr;
...
void AlphaCPU::execute(quint32 instr)  {
			auto op = decodeOperate(instr);
			switch (op.opcode) {
			case /*OP_ADD*/ 0x0: execADD(op); break;
			case /*OP_SUB*/ 0x1: execSUB(op); break;
				// … all your integer OP_… cases …
			default:
				ctx->notifyTrapRaised(helpers_JIT::TrapType::IllegalInstruction);
				break;
			}
		}
...
void AlphaCPU::executePALOperation(quint32 instruction)
{
    ...
    if (m_palInterpreter) {
        m_palInterpreter->processPALInstruction(palFunction);
    }
    else {
        raiseException(helpers_JIT::ExceptionType::ILLEGAL_INSTRUCTION, m_pc);
    }
...
void AlphaCPU::buildControlDispatchTable() {
	...
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BR)] = [this](quint32 instr) { controlExecutor->execBR(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BSR)] = [this](quint32 instr) { controlExecutor->execBSR(decodeBranch(instr)); };

	// Conditional Integer Branches
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BEQ)] = [this](quint32 instr) { controlExecutor->execBEQ(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BNE)] = [this](quint32 instr) { controlExecutor->execBNE(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BLT)] = [this](quint32 instr) { controlExecutor->execBLT(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BLE)] = [this](quint32 instr) { controlExecutor->execBLE(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BGT)] = [this](quint32 instr) { controlExecutor->execBGT(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BGE)] = [this](quint32 instr) { controlExecutor->execBGE(decodeBranch(instr)); };

	// Conditional Bit Branches
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BLBC)] = [this](quint32 instr) { controlExecutor->execBLBC(decodeBranch(instr)); };
	ctrlDispatch[static_cast<uint8_t>(CTRL::OP_CTRL_BLBS)] = [this](quint32 instr) { controlExecutor->execBLBS(decodeBranch(instr)); };
...
void AlphaCPU::executeNextInstruction()
{
    ...
    // 🛠 Step 4: Hot Spot Detection

    // Update execution counter
	if (!m_jitProfiler.isNull())
        m_jitProfiler->recordExecution(currentPC);
    ...
    if (m_jitEnabled && m_jitCompiler) {
		auto block = m_jitCompiler->compileOrGetBlock(m_pc);
		m_pc = block();  // Execute the compiled block
		return;
...
void AlphaCPU::onMemoryWritten(quint64 address, quint64 value, int size)
{
#ifdef QT_DEBUG
...
void AlphaCPU::onMemoryRead(quint64 vaddr, quint64 paddr, int size) {
	qDebug() << "[MemoryRead] vaddr:" << hex << vaddr << "paddr:" << paddr << "size:" << size;
}

void AlphaCPU::onMappingsCleared() {
	TRACE_LOG("[AlphaMemorySystem::onMappingsCleared()] Address translation mappings cleared.");
}


void AlphaCPU::onProtectionFault(quint64 vaddr, int accessType)
{
	QString accessStr = (accessType == 0) ? "READ" : (accessType == 1) ? "WRITE" : "EXECUTE";

	qCritical() << "[AlphaCPU] Protection fault at virtual address"
		<< QString("0x%1").arg(vaddr, 0, 16)
		<< "during" << accessStr << "access.";

	// TODO: Optional: raise a trap or halt CPU execution
	// raiseTrap(ExceptionType::MEMORY_ACCESS_VIOLATION);
}

void AlphaCPU::writeIntReg(int index, quint64 value)
{
	intRegisterBank.set(index, value);
	emit registerChanged(index, helpers_JIT::RegisterType::INTEGER_REG, value);
}

void AlphaCPU::onTranslationMiss(quint64 vaddr)
{
	qWarning() << "[AlphaCPU] Translation miss at virtual address"
		<< QString("0x%1").arg(vaddr, 0, 16);

	// TODO: Optional: attempt TLB refill or raise trap
	// raiseTrap(ExceptionType::PAGE_FAULT);
}
...
{
   // TODO - AlphaCPU::notifyReturnFromTrap()
}

#pragma endregion Event Wrappers


void AlphaCPU::writeFpcr(quint64 rawFpcr) {
	m_fpcr.setRaw(rawFpcr);
	//emit fpcrChanged(m_fpcr);
    //TODO AlphaCPU::writeFpcr(quint64 rawFpcr) {
}