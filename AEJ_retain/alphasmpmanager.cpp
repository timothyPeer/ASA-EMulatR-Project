...
void AlphaSMPManager::startAllCPUs_MoveToThread(quint64 pc_init)
{
	...
	for (int i = 0; i < m_cpus.size(); i++) {
		...
		connect(this, &AlphaSMPManager::signalSendInterrupt, alphaCPU, [=](int cpuId, int vector) {
			if (cpuId == i) alphaCPU->receiveInterrupt(cpuId,vector);
			});
...
void AlphaSMPManager::initializeCPUs(quint8	cpuCount_)
{
	...
	if (m_memorySystem) {
		m_memorySystem = new AlphaMemorySystem(m_safeMemory,m_memorySystem,this);
	}
	if (m_safeMemory)	{
	...
	if (m_systemLoader) {
		m_systemLoader = new SystemLoader(this);
	}
	if (m_irqController) {
		m_irqController = new IRQController(this);
	}
	quint16 cpuCnt = 1;
	if (m_cpus.count() == 0)
		cpuCnt = cpuCount_;
	...
	for (int i = 0; i < cpuCnt; i++) {
		if (!m_cpus[i])
		{
			m_cpus[i] = new AlphaCPU(i, m_memorySystem.data(), this);
			// set properties
			...
			connect(m_cpus[i], &AlphaCPU::cycleExecuted, this, [this, i](quint64 cycle) { // Optionally handle cycle completion	});
				...
				connect(this, &AlphaSMPManager::signalSendInterrupt, m_cpus[i], [=](int cpuId, int vector) {
					if (cpuId == i)
						m_cpus[i]->receiveInterrupt(cpuId,vector);
					});
				connect(m_cpus[i], &AlphaCPU::finished, m_moved_cpus[i], &QThread::quit);
				...
				connect(m_cpus[i], &AlphaCPU::executionStarted, this, &AlphaSMPManager::handleCoreStarted);
				connect(m_cpus[i], &AlphaCPU::executionStopped, this, &AlphaSMPManager::handleCoreStopped);
				connect(m_cpus[i], &AlphaCPU::executionPaused, this, &AlphaSMPManager::handleCorePaused);
			...
			TRACE_LOG(QString("SMP manager initialized with % CPUs").arg(m_cpus.size());
...
{
	//TODO registerDevice(DeviceInterface* device, quint64 mmioBase, quint64 mmioSize, int irq = -1

	Q_ASSERT(mmioManager && systemBus);

	mmioManager->mapDevice(device, mmioBase, mmioSize);
	systemBus->mapDevice(device, mmioBase, mmioSize);

	if (irq >= 0 && irqController)
		device->setIRQLine(irqController, irq);
...
{
	if (!m_memorySystem) {
		m_memorySystem.reset(new AlphaMemorySystem(this));
		m_memorySystem->setMemoryAlloc(ramSizeMB);
	}
