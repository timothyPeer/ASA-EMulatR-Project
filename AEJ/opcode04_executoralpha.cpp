#include "opcode04_executorAlpha.h"
...
opcode04_executorAlpha::opcode04_executorAlpha(AlphaCPU *cpu, QObject *parent) : QObject(parent), m_cpu(cpu)
{
    DEBUG_LOG("Creating Alpha Opcode 04 Executor");
    initialize();
}

opcode04_executorAlpha::~opcode04_executorAlpha()
{
    DEBUG_LOG("Destroying Alpha Opcode 04 Executor");
    stopAsyncPipeline();
}

void opcode04_executorAlpha::initialize()
{
    ...
    asa_utils::safeIncrement(m_opcode04Instructions, 0);
    ...
    DEBUG_LOG("Alpha Opcode 04 Executor initialized successfully");
}

void opcode04_executorAlpha::initialize_SignalsAndSlots()
{
    // Connect internal signals for pipeline optimization
    connect(this, &opcode04_executorAlpha::sigPipelineStalled, this, &opcode04_executorAlpha::handlePipelineStall,
            Qt::QueuedConnection);

    // Connect hot path optimization
    connect(this, &opcode04_executorAlpha::sigHotPathDetected, this, &opcode04_executorAlpha::optimizeHotPaths,
            Qt::QueuedConnection);
}

void opcode04_executorAlpha::startAsyncPipeline()
{
    ...
    DEBUG_LOG("Starting async Opcode 04 pipeline");
    ...
    DEBUG_LOG("Async Opcode 04 pipeline started successfully");
}

void opcode04_executorAlpha::stopAsyncPipeline()
{
    ...
    DEBUG_LOG("Stopping async Opcode 04 pipeline");
    ...
    DEBUG_LOG("Async Opcode 04 pipeline stopped");
}

bool opcode04_executorAlpha::submitInstruction(const DecodedInstruction &instruction, quint64 pc)
{
    ...
    quint64 seqNum = asa_utils::safeIncrement(m_sequenceCounter);
    Opcode04Instruction opInstr(instruction, pc, seqNum);
...
bool opcode04_executorAlpha::executeOpcode04(const DecodedInstruction &instruction)
{
    // Synchronous execution path - high performance fallback
    Opcode04Instruction instr(instruction, 0, 0);

    if (!decodeOpcode04Instruction(instr))
    {
        return false;
    }
    ...
    bool success = executeOpcode04Core(instr);

    if (success)
    {
        asa_utils::safeIncrement(m_syncExecutions);
        asa_utils::safeIncrement(m_opcode04Instructions);
    }
    else
    {
        asa_utils::safeIncrement(m_executionErrors);
...
bool opcode04_executorAlpha::decodeOpcode04Instruction(Opcode04Instruction &instr)
{
    quint32 raw32 = instr.instruction.raw;
    quint32 opcode = (raw32 >> 26) & 0x3F;

    // Verify this is opcode 0x04
    if (opcode != 0x04)
    {
        DEBUG_LOG("Invalid opcode for Opcode04 executor: 0x%02X", opcode);
        return false;
    }
...
void opcode04_executorAlpha::analyzeDependencies(Opcode04Instruction &instr)
{
    quint32 raw32 = instr.instruction.raw;
...
bool opcode04_executorAlpha::executeOpcode04Core(Opcode04Instruction &instr)
{
    ...
    case 0x07:
        ...
        break;
    default:
        DEBUG_LOG("Unknown Opcode 04 function: 0x%02X", function);
        success = false;
        break;
    }
    ...
    if (success)
    {
        ...
        emit sigOpcode04Executed(function, true);
    }
    else
    {
        emit sigOpcode04Executed(function, false);
    }

    return success;
}

// Function implementations - placeholder for actual Alpha opcode 04 functions
bool opcode04_executorAlpha::executeFunction00(const Opcode04Instruction &instr, quint64 &result)
{
    ...
    // Placeholder operation - implement actual Alpha function (Shift Left Logical)
    if (rbValue > 63) // Alpha shifts are limited to 0-63 positions
    {
        result = 0;
    }
    else
...
bool opcode04_executorAlpha::executeFunction01(const Opcode04Instruction &instr, quint64 &result)
{
    ...
    // Placeholder operation - Shift Right Logical
    if (rbValue > 63)
    {
        result = 0;
    }
...
bool opcode04_executorAlpha::executeFunction02(const Opcode04Instruction &instr, quint64 &result)
{
    ...
    // Placeholder operation - Shift Right Arithmetic
    if (rbValue > 63)
    {
        // For arithmetic shift, sign-extend
        result = (static_cast<qint64>(raValue) < 0) ? 0xFFFFFFFFFFFFFFFFULL : 0;
    }
    else
    {
        result = static_cast<quint64>(static_cast<qint64>(raValue) >> rbValue);
    }
    return true;
}

bool opcode04_executorAlpha::executeFunction03(const Opcode04Instruction &instr, quint64 &result)
{
    ...
    // Placeholder operation - Rotate Left
    rbValue &= 63; // Limit rotation to 0-63 positions
    result = (raValue << rbValue) | (raValue >> (64 - rbValue));
    return true;
}

bool opcode04_executorAlpha::executeFunction04(const Opcode04Instruction &instr, quint64 &result)
{
...
bool opcode04_executorAlpha::executeFunction05(const Opcode04Instruction &instr, quint64 &result)
{
...
bool opcode04_executorAlpha::executeFunction06(const Opcode04Instruction &instr, quint64 &result)
{
...
bool opcode04_executorAlpha::executeFunction07(const Opcode04Instruction &instr, quint64 &result)
{
...
// Pipeline Workers
void opcode04_executorAlpha::fetchWorker()
{
    while (m_pipelineActive.load())
    {
        ...
        {
            Opcode04Instruction instr = m_fetchQueue.dequeue();
...
void opcode04_executorAlpha::decodeWorker()
{
    while (m_pipelineActive.load())
    {
        QMutexLocker locker(&m_pipelineMutex);
        ...
        {
            Opcode04Instruction instr = m_decodeQueue.dequeue();

            // Decode is fast for opcode 04
            if (decodeOpcode04Instruction(instr))
            {
                instr.isReady = true;
...
void opcode04_executorAlpha::executeWorker()
{
    while (m_pipelineActive.load())
    {
        QMutexLocker locker(&m_pipelineMutex);
        ...
        {
            Opcode04Instruction instr = m_executeQueue.dequeue();
...
void opcode04_executorAlpha::writebackWorker()
{
    while (m_pipelineActive.load())
    {
        QMutexLocker locker(&m_pipelineMutex);
        ...
        if (!m_writebackQueue.isEmpty())
        {
            void opcode04_executorAlpha::writebackWorker()
            {
                while (m_pipelineActive.load())
                {
                    ...
                    if (!m_writebackQueue.isEmpty())
                    {
                        Opcode04Instruction instr = m_writebackQueue.dequeue();
            ...
            void opcode04_executorAlpha::warmupHotPath(quint64 pc, quint32 frequency)
            {
            ...
            bool opcode04_executorAlpha::isHotPath(quint64 pc) const
            {
                QMutexLocker locker(&m_hotPathMutex);
                return m_hotPathCache.contains(pc) && m_hotPathCache[pc] > 100;
            }

            void opcode04_executorAlpha::updateHotPathStats(quint64 pc)
            {
                QMutexLocker locker(&m_hotPathMutex);
            ...
            // JIT Optimization
            void opcode04_executorAlpha::jitOptimizeInstruction(Opcode04Instruction & instr)
            {
            ...
            bool opcode04_executorAlpha::executeJITOptimized(const Opcode04Instruction &instr, quint64 &result)
            {
                // Fast path for JIT-optimized instructions
                return executeOpcode04Core(const_cast<Opcode04Instruction &>(instr));
            }

            void opcode04_executorAlpha::cacheOptimizedPath(quint64 pc, const Opcode04Instruction &instr)
            {
            ...
            bool opcode04_executorAlpha::validateInstructionSafety(const Opcode04Instruction &instr) const
            {
            ...
            bool opcode04_executorAlpha::checkMemoryBounds(quint64 address, quint32 size) const
            {
            ...
            bool opcode04_executorAlpha::checkRegisterAccess(quint8 reg) const
            {
            ...
            bool opcode04_executorAlpha::checkDependencies(const Opcode04Instruction &instr) const
            {
                // Simple dependency check - in real implementation would be more sophisticated
                // For now, assume no dependencies for opcode 04 operations
                return true;
            }

            void opcode04_executorAlpha::updateDependencies(const Opcode04Instruction &instr)
            {
            ...
            bool opcode04_executorAlpha::fetchInstructionWithCache(quint64 pc, quint32 & instruction)
            {
            ...
            bool opcode04_executorAlpha::readRegisterWithCache(quint8 reg, quint64 & value)
            {
            ...
            bool opcode04_executorAlpha::writeRegisterWithCache(quint8 reg, quint64 value)
            {
                if (!m_cpu || !checkRegisterAccess(reg))
                {
                    return false;
            ...
            bool opcode04_executorAlpha::accessMemoryWithCache(quint64 address, quint8 * data, quint32 size,
                                                               bool isWrite)
            {
            ...
            void opcode04_executorAlpha::optimizePipelineFlow()
            {
            ...
            void opcode04_executorAlpha::adjustPipelineDepth()
            {
            ...
            void opcode04_executorAlpha::balanceWorkload()
            {
            ...
            // Error Handling
            void opcode04_executorAlpha::handleExecutionError(const Opcode04Instruction &instr, const QString &error)
            {
            ...
            void opcode04_executorAlpha::recoverFromPipelineStall()
            {
            ...
            bool opcode04_executorAlpha::validatePipelineIntegrity() const
            {
                QMutexLocker locker(&m_pipelineMutex);
            ...
            void opcode04_executorAlpha::printStatistics() const
            {
                QMutexLocker locker(&m_statsMutex);

                DEBUG_LOG("=== Alpha Opcode 04 Executor Statistics ===");
                DEBUG_LOG("Total Opcode 04 Instructions: %d", static_cast<int>(m_opcode04Instructions));
                ...
                DEBUG_LOG("\n=== Cache Performance ===");
                DEBUG_LOG("L1 I-Cache: Hits=%d, Misses=%d", static_cast<int>(m_l1ICacheHits),
                          static_cast<int>(m_l1ICacheMisses));
                DEBUG_LOG("L1 D-Cache: Hits=%d, Misses=%d", static_cast<int>(m_l1DCacheHits),
                          static_cast<int>(m_l1DCacheMisses));
                DEBUG_LOG("L2 Cache: Hits=%d, Misses=%d", static_cast<int>(m_l2CacheHits),
                          static_cast<int>(m_l2CacheMisses));
            ...
            void opcode04_executorAlpha::clearStatistics()
            {
                QMutexLocker locker(&m_statsMutex);
            ...
            qreal opcode04_executorAlpha::getPerformanceMetrics() const
            {
                if (m_totalExecutions == 0)
                {
                    return 0.0;
            ...
            // Slots
            void opcode04_executorAlpha::handlePipelineStall() { recoverFromPipelineStall(); }

            void opcode04_executorAlpha::optimizeHotPaths()
            {
            ...
            // Utility Methods
            quint32 opcode04_executorAlpha::extractFunction(quint32 rawInstruction) const
            {
                return (rawInstruction >> 5) & 0x7F; // 7-bit function code
            }

            quint8 opcode04_executorAlpha::extractRegisterA(quint32 rawInstruction) const
            {
                return (rawInstruction >> 21) & 0x1F; // bits 25:21
            }

            quint8 opcode04_executorAlpha::extractRegisterB(quint32 rawInstruction) const
            {
                return (rawInstruction >> 16) & 0x1F; // bits 20:16
            }

            quint8 opcode04_executorAlpha::extractRegisterC(quint32 rawInstruction) const
            {
                return rawInstruction & 0x1F; // bits 4:0
            }

            bool opcode04_executorAlpha::isLiteralMode(quint32 rawInstruction) const
            {
                return (rawInstruction >> 12) & 0x1; // bit 12
            }

            quint8 opcode04_executorAlpha::extractLiteral(quint32 rawInstruction) const
            {
