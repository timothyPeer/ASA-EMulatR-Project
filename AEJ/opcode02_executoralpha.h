...
// Opcode 02 instruction structure for pipeline
struct Opcode02Instruction
{
    ...
    Opcode02Instruction() = default;
    Opcode02Instruction(const DecodedInstruction &instr, quint64 programCounter, quint64 seqNum)
...
/**
 * @brief High-performance async opcode 0x02 executor
 *
 * Implements OpCode 0x02 operations with async pipeline, JIT optimization,
 * hot/warm path caching, and full memory/type safety.
 * Designed for maximum performance with Alpha CPU integration.
 */
class opcode02_executorAlpha : public QObject
{
    Q_OBJECT
  ...
  private:
    ...
    // Pipeline queues
    QQueue<Opcode02Instruction> m_fetchQueue;
    QQueue<Opcode02Instruction> m_decodeQueue;
    QQueue<Opcode02Instruction> m_executeQueue;
    QQueue<Opcode02Instruction> m_writebackQueue;
    ...
    QAtomicInt m_opcode02Instructions;
  ...
  public:
    explicit opcode02_executorAlpha(AlphaCPU *cpu, QObject *parent = nullptr);
    ~opcode02_executorAlpha();
    ...
    bool executeOpcode02(const DecodedInstruction &instruction);
    bool decodeOpcode02Instruction(Opcode02Instruction &instr);
  ...
  signals:
    void sigOpcode02Executed(quint32 function, bool success);
  ...
  private:
    ...
    bool executeOpcode02Core(Opcode02Instruction &instr);

    // Function-specific execution methods
    bool executeFunction00(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction01(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction02(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction03(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction04(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction05(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction06(const Opcode02Instruction &instr, quint64 &result);
    bool executeFunction07(const Opcode02Instruction &instr, quint64 &result);

    // JIT optimization methods
    void jitOptimizeInstruction(Opcode02Instruction &instr);
    bool executeJITOptimized(const Opcode02Instruction &instr, quint64 &result);
    void cacheOptimizedPath(quint64 pc, const Opcode02Instruction &instr);

    // Memory safety and type checking
    bool validateInstructionSafety(const Opcode02Instruction &instr) const;
    bool checkMemoryBounds(quint64 address, quint32 size) const;
    bool checkRegisterAccess(quint8 reg) const;

    // Dependency analysis
    void analyzeDependencies(Opcode02Instruction &instr);
    bool checkDependencies(const Opcode02Instruction &instr) const;
    void updateDependencies(const Opcode02Instruction &instr);
    ...
    void handleExecutionError(const Opcode02Instruction &instr, const QString &error);
