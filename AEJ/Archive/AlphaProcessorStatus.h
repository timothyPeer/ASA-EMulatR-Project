#pragma once
// =============================================================================
// ALPHA PROCESSOR STATUS IMPLEMENTATION
// =============================================================================
// Alpha AXP architecture processor status register implementation
// Based on Alpha Architecture Reference Manual, Fourth Edition



#include "../ABA/baseProcessContext.h"
...
class AlphaProcessorStatus
{
  ...
  public:
    ...
    explicit AlphaProcessorStatus(quint8 initialIPL = 0)
        : rawPS_(initialIPL & 0x7), mode_(ProcessorMode::MODE_USER), palMode_(false), flags_()
    {
    }
 ...
 /*   ProcessorStatusFlags calculateConditionCodes(qint64 result, qint64 op1, qint64 op2,*/
    ...
    void enterPALMode()
    {
        setFlag(enumProcessorStatus::PS_FLAG_SUPERVISOR_MODE, true);
    }

  /// Return true if specified status flag is set
    bool getFlag(enumProcessorStatus flag) const
    {
        return (statusWord_ & static_cast<quint64>(flag)) != 0;
    }
    ...
    bool isPALModeActive() const
    {
        return getFlag(enumProcessorStatus::PS_FLAG_SUPERVISOR_MODE);
    }
