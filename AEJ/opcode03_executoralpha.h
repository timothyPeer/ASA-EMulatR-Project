...
// Opcode 03 instruction structure for pipeline
struct Opcode03Instruction
{
    ...
    Opcode03Instruction() = default;
    Opcode03Instruction(const DecodedInstruction &instr, quint64 programCounter, quint64 seqNum)
...
/**
 * @brief High-performance async opcode 0x03 executor
 *
 * Implements OpCode 0x03 operations with async pipeline, JIT optimization,
 * hot/warm path caching, and full memory/type safety.
 * Designed for maximum performance with Alpha CPU integration.
 */
class opcode03_executorAlpha : public QObject
{
    Q_OBJECT
  ...
  private:
    ...
    // Pipeline queues
    QQueue<Opcode03Instruction> m_fetchQueue;
    QQueue<Opcode03Instruction> m_decodeQueue;
    QQueue<Opcode03Instruction> m_executeQueue;
    QQueue<Opcode03Instruction> m_writebackQueue;
    ...
    QAtomicInt m_opcode03Instructions;
  ...
  public:
    explicit opcode03_executorAlpha(AlphaCPU *cpu, QObject *parent = nullptr);
    ~opcode03_executorAlpha();
    ...
    bool executeOpcode03(const DecodedInstruction &instruction);
    bool decodeOpcode03Instruction(Opcode03Instruction &instr);
  ...
  signals:
    void sigOpcode03Executed(quint32 function, bool success);
  ...
  private:
    ...
    bool executeOpcode03Core(Opcode03Instruction &instr);

    // Function-specific execution methods
    bool executeFunction00(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction01(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction02(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction03(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction04(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction05(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction06(const Opcode03Instruction &instr, quint64 &result);
    bool executeFunction07(const Opcode03Instruction &instr, quint64 &result);

    // JIT optimization methods
    void jitOptimizeInstruction(Opcode03Instruction &instr);
    bool executeJITOptimized(const Opcode03Instruction &instr, quint64 &result);
    void cacheOptimizedPath(quint64 pc, const Opcode03Instruction &instr);

    // Memory safety and type checking
    bool validateInstructionSafety(const Opcode03Instruction &instr) const;
    bool checkMemoryBounds(quint64 address, quint32 size) const;
    bool checkRegisterAccess(quint8 reg) const;

    // Dependency analysis
    void analyzeDependencies(Opcode03Instruction &instr);
    bool checkDependencies(const Opcode03Instruction &instr) const;
    void updateDependencies(const Opcode03Instruction &instr);
    ...
    void handleExecutionError(const Opcode03Instruction &instr, const QString &error);
