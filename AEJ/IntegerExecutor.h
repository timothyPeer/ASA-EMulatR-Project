...
public:
	...
	void execDIV(const OperateInstruction& op) {
		...
		if (divisor == 0) {
			ctx->notifyTrapRaised(helpers_JIT::TrapType::DivideByZero_int);
			return;
		}
	...
	void execLDB(const OperateInstruction& op) {
		...
		if (!memSystem->readVirtualMemory(alphaCPU, addr, temp, 1)) {
			ctx->notifyTrapRaised(helpers_JIT::TrapType::MMUAccessFault);
			return;
		}
	...
	void execSTB(const OperateInstruction& op) {
		quint64 addr = regs->readIntReg(op.ra) + regs->readIntReg(op.rb);
		quint8 value = static_cast<quint8>(regs->readIntReg(op.rc));
	...
	void execSTW(const OperateInstruction& op) {
		quint64 addr = regs->readIntReg(op.ra) + regs->readIntReg(op.rb);
		quint16 value = static_cast<quint16>(regs->readIntReg(op.rc));
